...
public class LoginDAOImpl implements LoginDAO {
    ...
    public LoginDTO login(User user) throws SQLException {
        ...
        String sql = "SELECT account_type,user_id  FROM app_user WHERE user_email=? AND password=? AND account_status=? AND is_delete=0";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    loginDTO.setUserId(rs.getInt("user_id"));
                    return loginDTO;
                }
            }
        } 
