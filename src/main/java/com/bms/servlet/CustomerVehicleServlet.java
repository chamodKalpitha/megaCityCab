...
public class CustomerVehicleServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
			...
			List<VehicleDTO> vehicles = controller.getVehiclesForCustomer(searchQuery, entries, offset);
			
			request.setAttribute("vehicles", vehicles);
	        request.setAttribute("page", currentPage);
	        request.setAttribute("count", controller.getVehiclesCountForCustomer(searchQuery));
	        request.setAttribute("entries", entries);
	        request.setAttribute("search", searchQuery);
	        
	        request.getRequestDispatcher("/customer/vehicles.jsp").forward(request, response);
